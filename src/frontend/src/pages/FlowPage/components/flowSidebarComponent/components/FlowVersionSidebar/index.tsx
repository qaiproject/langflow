    ...
    <>
      <div className="flex h-full flex-col">
        <SidebarGroupLabel className="flex items-center justify-between px-3 pt-3">
          <span>{t("flowSidebar.versionHistory")}</span>
        ...
        {isEntryError && (
          <div className="flex items-center gap-2 bg-destructive/10 px-2 py-2">
            <span className="text-xs text-destructive">
              {t("flowVersions.failedLoadVersionData")}
            </span>
          </div>
        )}
        ...
        {processedPreview?.error && (
          <div className="flex items-center gap-2 bg-destructive/10 px-2 py-2">
            <span className="text-xs text-destructive">
              {t("flowVersions.failedPreviewVersionData")}
            </span>
          </div>
        )}
          ...
          <SidebarMenu className="gap-0">
                ...
                <div className="flex w-full items-center justify-between">
                  <div className="flex flex-col items-start">
                    <span className="font-medium text-sm pb-1">
                      {t("flowVersions.current")}
                    </span>
                    <span className="text-xs text-muted-foreground">
                      {t("flowVersions.workingVersion")}
                    </span>
                  </div>
                  {isViewingDraft && (
            ...
            {isListError && (
              <div className="px-2 py-6 text-center text-xs text-destructive">
                {t("flowVersions.failedLoadVersions")}
              </div>
            )}
            {!isLoading &&
              !isListError &&
              (!versions || versions.length === 0) && (
                <div className="px-2 py-6 text-center text-xs text-muted-foreground">
                  {t("flowVersions.noSavedVersionsYet")}
                </div>
              )}
