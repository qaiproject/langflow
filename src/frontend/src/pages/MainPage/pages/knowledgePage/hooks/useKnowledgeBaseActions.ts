...
export const useKnowledgeBaseActions = ({
  ...
  const cancelIngestionMutation = useCancelIngestion({
    onSuccess: () => {
      setSuccessData({ title: t("knowledge.ingestionCancelled") });
      refetch();
    },
    onError: (error: AxiosError<{ detail?: string }>) => {
      setErrorData({
        title: t("knowledge.cancelIngestionFailed"),
        list: [
  ...
  const deleteKnowledgeBaseMutation = useDeleteKnowledgeBase({
    onSuccess: () => {
      setSuccessData({ title: t("knowledge.deletedSingle") });
    },
    onError: (error: AxiosError<{ detail?: string }>) => {
      setErrorData({
        title: t("knowledge.deleteSingleFailed"),
        list: [
          error?.response?.data?.detail ||
            error?.message ||
            t("knowledge.unknownError"),
  ...
  const deleteKnowledgeBasesMutation = useDeleteKnowledgeBase({
    onSuccess: () => {
      setSuccessData({ title: t("knowledge.deletedPlural") });
    },
    onError: (error: AxiosError<{ detail?: string }>) => {
      setErrorData({
        title: t("knowledge.deletePluralFailed"),
        list: [
          error?.response?.data?.detail ||
            error?.message ||
            t("knowledge.unknownError"),
