    ...
    <>
      <Form.Field name="name">
        <div className="edit-flow-arrangement">
          ...
          {isMaxLength && (
            <span className="edit-flow-span">{t("flow.charLimitReached")}</span>
          )}
          {isMinLength && (
            <span className="edit-flow-span">
              {t("flow.minCharsRequired", { count: minLength })}
            </span>
          )}
          {isInvalidName && (
            <span className="edit-flow-span">{t("flow.nameExists")}</span>
          )}
        </div>
        {setName ? (
      ...
      <Form.Field name="description">
        <div className="edit-flow-arrangement mt-2">
          <Form.Label className="text-mmd font-medium">
            {t("flow.description")}{setDescription ? "" : ":"}
          </Form.Label>
        ...
        {setDescription ? (
          <Form.Control asChild>
            <Textarea
              ...
              value={description!}
              placeholder={t("flow.flowDescription")}
