  ...
  return (
    ...
    >
      <span className="flex w-full items-center text-sm">
        {t("voiceAssistant.audioInput")}
        <ShadTooltip content={t("voiceAssistant.microphoneTooltip")}>
          <div>
            <IconComponent
              name="Info"
              strokeWidth={2}
      ...
      <Select value={selectedMicrophone} onValueChange={handleSetMicrophone}>
        <SelectTrigger className="h-9 w-full">
          <SelectValue placeholder={t("voiceAssistant.selectMicrophone")} />
        </SelectTrigger>
          ...
          <SelectGroup>
            {microphones?.map((device) => (
              <SelectItem key={device?.deviceId} value={device?.deviceId}>
                <div className="max-w-[220px] truncate text-left">
                  {device?.label ||
                    t("voiceAssistant.microphoneFallback", {
            ...
            {microphones?.length === 0 && (
              <SelectItem value="no-microphones" disabled>
                {t("voiceAssistant.noMicrophonesFound")}
              </SelectItem>
            )}
          </SelectGroup>
        </SelectContent>
