...
const SwitchOutputView: React.FC<SwitchOutputViewProps> = ({
  ...
  const ToolOutputDisplay = ({ tools }) => {
    if (!Array.isArray(tools) || tools.length === 0) {
      return <div>{t("outputModal.noToolsAvailable")}</div>;
    }
  ...
  return type === "Outputs" ? (
    <>
      ...
      >
        <div>{t("outputModal.noOutput")}</div>
      </Case>
      <Case
        condition={
      ...
      <Case condition={resultType === "stream" && !isToolOutput}>
        <div className="flex h-full w-full items-center justify-center align-middle">
          <Alert variant={"default"} className="w-fit">
            ...
            />
            <AlertTitle>{t("outputModal.streamingNotSupported")}</AlertTitle>
            <AlertDescription>
              {t("outputModal.usePlaygroundForStreaming")}
            </AlertDescription>
          </Alert>
        </div>
      </Case>
