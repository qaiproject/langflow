...
export default function OutputModal({
  ...
  const handleCopy = () => {
    ...
    navigator.clipboard.writeText(content).then(() => {
      setIsCopied(true);
      setSuccessData({ title: t("outputModal.copiedToClipboard") });
      setTimeout(() => {
        setIsCopied(false);
      }, 2000);
    });
  ...
  return (
    <BaseModal
      ...
      className="z-50"
    >
      <BaseModal.Header description={t("outputModal.description")}>
        <div
          className="flex items-center"
          data-testid={`${nodeId}-${outputName}-output-modal`}
        >
          <span className="pr-2">{t("outputModal.title")}</span>
        </div>
      ...
      <BaseModal.Content>
        ...
        >
          <TabsList>
            <TabsTrigger value="Outputs">{t("outputModal.outputs")}</TabsTrigger>
            <TabsTrigger value="Logs">{t("outputModal.logs")}</TabsTrigger>
          </TabsList>
        </Tabs>
